...
#include "fwd_bwd_rotation_shim_controller/fwd_bwd_rotation_shim_controller.hpp"

using rcl_interfaces::msg::ParameterType;

namespace fwd_bwd_rotation_shim_controller
{
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  auto ret = rcutils_logging_set_logger_level(
    logger_.get_name(), RCUTILS_LOG_SEVERITY_INFO);
  if (ret != RCUTILS_RET_OK) {
    RCLCPP_ERROR(logger_, "Error setting severity: %s", rcutils_get_error_string().str);
    rcutils_reset_error();
  }
}
  ...
  nav2_core::GoalChecker * goal_checker)
{
  if (path_updated_) {
    ...
    try {
      ...
      // Extention to fwd_bwd operation -- If robot points in counter-goal direction
      // orient the robot to drive backwards
      if (fabs(angular_distance_to_heading) > M_PI_2) {
        double angular_distance_to_heading_fwd = angular_distance_to_heading;
        if (angular_distance_to_heading > 0.0) {
        ...
        RCLCPP_DEBUG(
          logger_,
          "Robots back points to goal --> drive backwards --> heading angle changed from %0.3f --> %0.3f",
          angular_distance_to_heading_fwd, angular_distance_to_heading);
      }
...
PLUGINLIB_EXPORT_CLASS(
  fwd_bwd_rotation_shim_controller::RotationShimController,
